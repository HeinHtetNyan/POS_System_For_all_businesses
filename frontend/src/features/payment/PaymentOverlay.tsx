...
const METHODS: { id: PaymentMethod; labelKey: string; icon: typeof IconCash; activeClass: string }[] = [
  { id: 'cash',  labelKey: 'payment.cash',         icon: IconCash,  activeClass: 'bg-amber-500/20 border-amber-500/50 text-amber-400' },
  { id: 'card',  labelKey: 'payment.digital_card', icon: IconCard,  activeClass: 'bg-blue-500/20 border-blue-500/50 text-blue-400' },
  { id: 'split', labelKey: 'payment.split',        icon: IconSplit, activeClass: 'bg-violet-500/20 border-violet-500/50 text-violet-400' },
]
...
export default function PaymentOverlay() {
  ...
  async function doProcess() {
    ...
    if (!activeSession) {
      toast.error(t('payment.no_active_session'))
    ...
    try {
      ...
      if (err instanceof OfflineError) {
        toast.error(t('payment.offline_blocked'))
      ...
      if (axios.isAxiosError(err) && !err.response) {
        try {
          ...
          toast.info(t('payment.sale_saved_offline'), { duration: 6000 })
        } catch {
          toast.error(t('payment.sale_save_failed'))
          setCheckoutStep('payment')
        }
        return
      }

      const msg = extractApiMsg(err) ?? t('payment.checkout_failed')
      toast.error(msg)
      setCheckoutStep('payment')
    } finally {
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800 flex-shrink-0">
          <div className="flex items-center gap-3">
            <p className="text-base font-bold text-zinc-100">{t('payment.title')}</p>
            <span className="text-xs text-zinc-600">{totals.itemCount} {t('payment.items_suffix')}</span>
          </div>
          <div className="flex items-center gap-3">
            <span className="font-mono text-lg font-bold text-amber-400">{fmt(totals.total)}</span>
            <button
              onClick={() => !isProcessing && setCheckoutStep('cart')}
              aria-label={t('payment.back_to_cart')}
      ...
      {(checkoutStep === 'processing' || isProcessing) && (
        <div className="fixed inset-0 z-[60] bg-zinc-950/95 flex flex-col items-center justify-center gap-4">
          <Spinner size={48} />
          <p className="text-sm font-semibold text-zinc-400 animate-pulse">{t('payment.processing')}</p>
        </div>
      )}
    </>
  )
